...
final class LoginAdminController extends Controller
{
    ...
    {
        $arquivoConexao = APP_ROOT . '/database/conexao.php';

        if (!is_file($arquivoConexao)) {
            throw new RuntimeException(
                'Arquivo de conexão não encontrado: ' . $arquivoConexao
            );
        }

        $pdo = require $arquivoConexao;
        ...
        $this->usuarios = new UsuarioAdminRepository($pdo);
    }
    ...
    public function formulario(): void
    {
        ...
        $this->view(
            'site/loginadmin',
            [
                'tituloPagina' => 'Login administrativo',
                'erro' => $erro,
                'email' => $email,
    ...
    public function autenticar(): void
    {
        ...
        $usuario = $this->usuarios->buscarAtivoPorEmail($email);
        ...
        if (
            ...
            $this->usuarios->atualizarHashSenha(
                (int) $usuario['id'],
                $novoHash
            );
        }
        ...
        $this->usuarios->registrarUltimoAcesso(
            (int) $usuario['id']
        );
